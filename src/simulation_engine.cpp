...
// OpenBLAS header for cblas_ddot
extern "C" {
#include <cblas.h>
}
...
std::vector<ProcessedVariant> SimulationEngine::simulateVariants(
    ...
    // Pre-size the results vector to allow for thread-safe, index-based insertion
    std::vector<ProcessedVariant> results(variants.size());
    std::atomic<size_t> completed_count{0};

    // Enqueue all the simulation tasks
    {
        std::unique_lock<std::mutex> lock(queue_mutex_);
        for (size_t i = 0; i < variants.size(); ++i) {
            tasks_.emplace([this, &variants, i, &results, &completed_count]() {
                // Each task processes one variant and places it in the correct output slot
                results[i] = processVariant(*variants[i], i);
                
                // Atomically increment the counter and log progress
                size_t done = completed_count.fetch_add(1) + 1;
                if (done % 10000 == 0 || done == variants.size()) {
                    logInfo("Simulated " + std::to_string(done) + "/" +
    ...
    while (completed_count.load() < variants.size()) {
